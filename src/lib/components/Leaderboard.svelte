<script lang="ts">
	...
	let displayMembers = $state([]);

	const oldRanks = data.diffRanks;

	const sortedByOldRank = [...sortedMembers].sort((a, b) => {
		const rankA = oldRanks.get(a.id) ?? 999;
		const rankB = oldRanks.get(b.id) ?? 999;
		return rankA - rankB;
	});
	...
	for (let i = 0; i < sortedMembers.length; i++) {
		...
		const previousRank = oldRanks.get(sortedMembers[i].id) ?? currentRank;
